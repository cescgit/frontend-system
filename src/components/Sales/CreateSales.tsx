...
export default function CreateSales({ dataAuth }: { dataAuth: AuthPermissions }) {
    ...
    const [dataProducts, setDataProducts] = useState<TempPurchasingFormData[]>(sessionStorage.getItem("tempSalesProductsAdd") ? JSON.parse(sessionStorage.getItem("tempSalesProductsAdd")!) : [])
    ...
    useEffect(() => {
        const storedProducts = sessionStorage.getItem("tempSalesProductsAdd");
        if (storedProducts) {
    ...
    useEffect(() => {
        sessionStorage.setItem("tempSalesProductsAdd", JSON.stringify(dataProducts));
    }, [dataProducts])
    ...
    function handleCalculateTotal() {
        ...
        newSales.subtotal = sum.toString();
    ...
    const handleCalculateTotalWithTaxes = () => {
        ...
        newSales.total = result.toString();
        setTotal(result)
    }
    ...
    const handleSelectionCustomer = (dataCustomer: CustomerFormDataInfo) => {
        ...
        setNewSales({ ...newSales, id_cliente: dataCustomer.id })
    }
    ...
    const {
        ...
        reset,
        formState: { errors },
    } = useForm<SalesFormDataAdd>({ defaultValues: newSales });
            ...
            <Dialog.Root open={open} onOpenChange={setOpen}>
                <Dialog.Trigger asChild>
                    <button
                        ...
                        onClick={() =>
                            navigate(location.pathname + "?newBilling")
                        }
                    >
                    ...
                    <Dialog.Content
                        ...
                        <div className="flex mt-4 items-center justify-center md:gap-x-8">
                            ...
                            >
                                <button
                                    type="submit"
                                    className={`w-full md:w-auto border border-gray-300 py-2 px-4 bg-slate-50/85 rounded-md flex items-center justify-center gap-x-4 font-bold hover:bg-slate-100/85 transition-all duration-200 ${customerData == undefined || customerData?.nombre_cliente == "" ? "cursor-not-allowed" : ""}`}
                                    aria-label="Close"
                                    onClick={() => {
                                        setTimeout(() => {
                                            if (!errors) {
                                    ...
                                    disabled={customerData == undefined || customerData?.nombre_cliente == "" ? true : false}
                                >
