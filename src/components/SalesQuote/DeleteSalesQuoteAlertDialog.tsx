...
import { cancelSalesQuote } from "../../apis/SalesQuoteAPI";
import Swal from "sweetalert2";
import withReactContent from 'sweetalert2-react-content'

const MySwal = withReactContent(Swal);

export default function DeleteSalesQuoteAlertDialog({ salesQuote, onClose }: { salesQuote: SalesQuoteFormDataDelete, onClose: () => void }) {
    const [deleteFormSalesQuote] = useState(salesQuote)
    const queryClient = useQueryClient()

    const [open, setOpen] = useState(true);

    const { mutate } = useMutation({
        mutationFn: cancelSalesQuote,
        onError: (error) => {
            MySwal.fire({
                position: "center",
                title: "Proforma",
                ...
                <AlertDialog.Portal>
                    ...
                    <AlertDialog.Content className={`fixed left-1/2 top-1/2 max-h-[85vh] w-[90vw] max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        <AlertDialog.Title className="font-bold text-center text-lg">
                            ¿Seguro desea anular la proforma?
                        </AlertDialog.Title>
                        <AlertDialog.Description className="my-2 text-base text-center text-gray-600">
                            {
                                <p>
                                    La proforma número {" "}
                                    <span className="font-bold">{deleteFormSalesQuote.numero_cotizacion}</span>,
                                    se anulara si haces clic en <br />
                                    <span className="bg-red-200 px-1">Si, anular proforma...</span>
                                </p>
                            }
                        ...
                        <div className="flex flex-col md:flex-row justify-center md:justify-end gap-y-4 md:gap-x-10">
                            ...
                            <AlertDialog.Action asChild>
                                <button
                                    ...
                                    onClick={() => {
                                        mutate(deleteFormSalesQuote.id)
                                        onClose()
                                    }}
                                >
                                    ¡Si, Eliminarlo!
