...
export default function ModalViewSales({ sales }: { sales: SalesFormDataInfo }) {
    ...
    const salesId = queryParams.get("viewSales")!
    const id = salesId;

    const [dataProducts, setDataProducts] = useState<TempSalesFormDataDetails[]>(sessionStorage.getItem("tempProductsViewSales") ? JSON.parse(sessionStorage.getItem("tempProductsViewSales")!) : [])
    const [customerIdValue, setCustomerIdValue] = useState<CustomerFormDataInfo | null>(null)

    const { isError, data } = useQuery({
        queryKey: ["sales", salesId],
        queryFn: () => getDetailsSalesById({ id }),
        enabled: !!salesId,
        retry: false
    })    
    ...
    useEffect(() => {
        if (data) {
            sessionStorage.setItem("tempProductsViewSales", JSON.stringify(data));
            const storedProducts = sessionStorage.getItem("tempProductsViewSales");
            if (storedProducts) {
    ...
    useEffect(() => {
        ...
        if (dataCustomer) {
            ...
            dataCustomer?.map(customerData => {
                if (customerData.id === sales.id_cliente) {
    ...
    }, [sales.id_cliente, dataCustomer, customerIdValue])
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className={`fixed left-1/2 top-1/2 h-[98%] w-full sm:w-[96%] sm:mx-auto px-2 py-4 md:p-6 touch-pan-x touch-pan-y scroll-smooth overflow-scroll -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                    <Dialog.Title className="font-bold text-black text-2xl text-center">
                        Vista de facturación <span className="text-red-500">{sales.numero_venta}</span>
                    </Dialog.Title>
                    <Dialog.Description className="my-2 text-base text-black text-center">
                        Datos de la facturación {sales.numero_venta} el día {formatDate(sales.fecha_creacion)}...
                    </Dialog.Description>
                                ...
                                <div className="flex flex-1 w-full items-center justify-center">
                                    <div className="w-full">
                                        ...
                                        <textarea
                                            ...
                                            defaultValue={sales.observaciones}
                                            disabled
                                        />
                                    </div>
                                </div>
                        ...
                        <div className="w-full lg:w-[25%] flex items-end gap-y-2 flex-col mt-2 border border-gray-500 rounded-lg py-4 px-2 h-auto touch-pan-y scroll-smooth">
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                ...
                                <input
                                    ...
                                    value={formatCurrency(sales.subtotal)}
                                    type="text"
                                    disabled
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                ...
                                <input
                                    id="value_tax"
                                    className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 cursor-not-allowed"
                                    value={formatCurrency(sales.impuesto_manual![0].valor_cantidad.toString())}
                                    type="text"
                                    disabled
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                ...
                                <input
                                    id="impuesto"
                                    className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 cursor-not-allowed"
                                    value={sales.impuesto_manual![0].porcentaje}
                                    disabled
                                    type="text"
                                    placeholder="Impuesto..." />
                            </div>
