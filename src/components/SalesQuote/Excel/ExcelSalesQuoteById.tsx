...
export default function ExcelSalesQuoteById({ salesQuote }: { salesQuote: SalesQuoteFormDataInfo }) {

  const [isDownloading, setIsDownloading] = useState(false)

  const downloadPdf = async () => {
    try {
      const response = await api(`salesQuote/excelSalesQuote/createExcel/${salesQuote.id}`, {
        responseType: "blob",
      });
      ...
      link.download = `Proforma-${salesQuote.numero_cotizacion}-${salesQuote.cliente}.xlsx`; // Nombre del archivo a descargar
      link.click();
