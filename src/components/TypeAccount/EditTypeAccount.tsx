...
import { TypeAccountFormDataEdit } from "../../types/typeAccontData";
import { getTypeAccountById, updateTypeAccount } from "../../apis/TypeAccountAPI";
import Swal from "sweetalert2";
import withReactContent from 'sweetalert2-react-content'

const MySwal = withReactContent(Swal);

export default function EditTypeAccount({ typeAccount, onClose }: { typeAccount: TypeAccountFormDataEdit, onClose: () => void }) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const typeAccountId = queryParams.get("editTypeAccount")!
    const id = typeAccountId;
    let idUSerLogin;

    const { isError, data } = useQuery({
        queryKey: ["typeAccount", typeAccountId],
        queryFn: () => getTypeAccountById({ id }),
        enabled: !!typeAccountId,
        retry: false
    })

    const [open, setOpen] = useState(false);
    const [editeTypeAccount, setEditeTypeAccount] = useState(typeAccount)
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<TypeAccountFormDataEdit>({ defaultValues: typeAccount });


    const { mutate } = useMutation({
        mutationFn: updateTypeAccount,
        onError: (error) => {
            MySwal.fire({
                position: "center",
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ["typeAccount"] })
            queryClient.invalidateQueries({ queryKey: ["editTypeAccount", typeAccountId] })
            MySwal.fire({
                position: "center",
                title: "Tipo de cuenta",
                icon: "success",
    ...
    const onSubmitEdit = () => {
        ...
        editeTypeAccount!.usuario_modificador = idUSerLogin;

        const formData = editeTypeAccount;
        ...
        const dataBrand = { typeAccountId, formData }
        mutate(dataBrand)
    }
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className={`fixed left-1/2 top-1/2 max-h-[90vh] overflow-y-auto touch-pan-y w-[90vw] max-w-[550px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                    ...
                    <Dialog.Description className="my-2 text-base text-black text-center">
                        Modifica los tipo de cuenta aquí...
                    </Dialog.Description>
                    <form
                    ...
                    >
                        <div className="w-full">
                            ...
                            <input
                                ...
                                id="nombre"
                                value={editeTypeAccount.nombre}
                                onChange={(e) => {
                                    setEditeTypeAccount({ ...editeTypeAccount, nombre: e.target.value });
                                }}
                                minLength={5}
                                maxLength={50}
                            />
                        ...
                        <div className="w-full">
                            ...
                            <input
                                ...
                                id="descripcion"
                                value={editeTypeAccount.descripcion}
                                onChange={(e) => {
                                    setEditeTypeAccount({ ...editeTypeAccount, descripcion: e.target.value });
                                }}
                                minLength={5}
                                maxLength={150}
                            />
