...
import { PreInvoicingFormDataInfo, TempPreInvoicingFormDataDetails } from "../../types/preInvoicingData";
import { getDetailsPreInvoicingById } from "../../api/PreInvoicingAPI";


export default function ModalViewpreInvoicing({ preInvoicing }: { preInvoicing: PreInvoicingFormDataInfo }) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const preInvoicingId = queryParams.get("viewPreInvoicing")!
    const id = preInvoicingId;

    const [dataProducts, setDataProducts] = useState<TempPreInvoicingFormDataDetails[]>(sessionStorage.getItem("tempProductsViewPreInvoicing") ? JSON.parse(sessionStorage.getItem("tempProductsViewPreInvoicing")!) : [])
    const [customerIdValue, setCustomerIdValue] = useState<CustomerFormDataInfo | null>(null)

    const { isError, data } = useQuery({
        queryKey: ["preInvoicing", preInvoicingId],
        queryFn: () => getDetailsPreInvoicingById({ id }),
        enabled: !!preInvoicingId,
        retry: false
    })

    useEffect(() => {
        if (data) {
            sessionStorage.setItem("tempProductsViewPreInvoicing", JSON.stringify(data));
            const storedProducts = sessionStorage.getItem("tempProductsViewPreInvoicing");
            if (storedProducts) {
    ...
    useEffect(() => {
        ...
        if (dataCustomer && preInvoicing.id_cliente != null || preInvoicing.id_cliente !== "") {
            dataCustomer?.map(customerData => {
                if (customerData.id === preInvoicing.id_cliente) {
    ...
    }, [preInvoicing.id_cliente, dataCustomer, customerIdValue])
    ...
    return (
        <>
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 h-[95%] w-full sm:w-[96%] sm:mx-auto px-2 py-4 md:p-6 touch-pan-x touch-pan-y scroll-smooth overflow-scroll -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=open]:animate-contentShow">
                    <Dialog.Title className="font-bold text-black text-2xl text-center">
                        Vista de prefacturación <span className="text-red-500">{preInvoicing.numero_prefacturacion}</span>
                    </Dialog.Title>
                    <Dialog.Description className="my-2 text-base text-black text-center">
                        Datos de la prefacturación {preInvoicing.numero_prefacturacion} el día {formatDate(preInvoicing.fecha_creacion)}...
                    </Dialog.Description>
                    ...
                    <div className="border border-gray-600 rounded-md h-auto mx-auto touch-pan-y scroll-smooth w-full px-1 sm:p-4">
                        <div className="flex flex-col md:flex-row items-start justify-start gap-x-4">
                                ...
                                <div className="flex flex-1 w-full items-center justify-center">
                                    <div className="w-full">
                                        ...
                                        <textarea
                                            ...
                                            defaultValue={preInvoicing.observaciones}
                                            disabled
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-1 w-full items-center justify-center">
                                    <div className="w-full">
                                        ...
                                        <input
                                            id="termino"
                                            className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 cursor-not-allowed"
                                            defaultValue={preInvoicing.termino}
                                            disabled
                                            type="text"
                                        />
                                    </div>
                            ...
                            <div className="w-full">
                                {
                                    preInvoicing.id_cliente == null || preInvoicing.id_cliente == "" || preInvoicing.id_cliente == undefined ?
                                        (
                                                ...
                                                <div className="w-full flex items-center justify-center flex-col">
                                                    <div className="w-full mt-4 gap-Y-2 flex-col flex items-start">
                                                        ...
                                                        <input
                                                            className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 cursor-not-allowed"
                                                            id="nombre_proveedor"
                                                            defaultValue={preInvoicing.cliente_manual![0].nombre_cliente}
                                                            type="text"
                                                            disabled
                                                        />
                                                    </div>
                    ...
                    <div className="h-auto w-full lg:w-full flex-col lg:flex-row flex md:gap-x-4 gap-y-4 items-start justify-center mx-auto">
                        ...
                        <div className="w-full lg:w-[25%] flex items-end gap-y-2 flex-col mt-2 border border-gray-500 rounded-lg py-4 px-2 h-auto touch-pan-y scroll-smooth">
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                <label htmlFor="subtotal_factura" className="font-bold text-gray-600 w-full">Subtotal de factura:</label>
                                <input
                                    id="subtotal_factura"
                                    ...
                                    value={formatCurrency(preInvoicing.subtotal)}
                                    type="text"
                                    disabled
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                ...
                                <input
                                    id="value_tax"
                                    className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 cursor-not-allowed"
                                    value={formatCurrency(preInvoicing.impuesto_manual![0].valor_cantidad.toString())}
                                    type="text"
                                    disabled
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                ...
                                <input
                                    id="impuesto"
                                    className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 cursor-not-allowed"
                                    value={preInvoicing.impuesto_manual![0].porcentaje}
                                    disabled
                                    type="text"
                                    placeholder="Impuesto..." />
                            </div>
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                <label htmlFor="total_factura" className="font-bold text-gray-600 w-full ">Total de factura:</label>
                                <input
                                    id="total_factura"
                                    className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 text-green-800 cursor-not-allowed"
                                    value={formatCurrency(preInvoicing.total)}
                                    type="text"
                                    disabled
