...
export default function ProductsKardexComboBox({ onSelectionChange }: { onSelectionChange: (product: ProductDataCombobox) => void }) {
    const navigate = useNavigate();
        const location = useLocation()
        const queryParams = new URLSearchParams(location.search)
        const viewKardex = queryParams.get("viewKardex")!;
    ...
    const handleSelect = (products: ProductDataCombobox) => {
        ...
        <>
            <Dialog.Root open={open} onOpenChange={setOpen}>
                ...
                <Dialog.Portal>
                    ...
                    <Dialog.Content className={`w-[90%] fixed left-1/2 top-1/2 overflow-y-auto scroll-smooth touch-pan-y overflow-scroll -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        <Dialog.Title className="font-bold text-black text-2xl text-center">
                            Selección de producto para ver movimientos
                        </Dialog.Title>
                        <Dialog.Description className="my-2 text-base text-black text-center">
                            Selecciona el producto para mostrar movimientos de entradas y salidas...
                        </Dialog.Description>
                                ...
                                <Table.Body>
                                    {
                                        filteredProducts?.map(product => (
                                            <Table.Row align="center" key={product.id} className="hover:bg-gray-100/85 transition-all duration-200">
                                                ...
                                                {
                                                    <Table.Cell>
                                                        <Badge radius="large" color={`${product.estado == 1 ? "green" : "red"}`}>
