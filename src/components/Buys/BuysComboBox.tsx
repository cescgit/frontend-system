...
type BuysProps = {
    onSelectionChange: (buys: BuysFormDataInfo) => void;
  };

export default function BuysComboBox({ onSelectionChange }: BuysProps) {    
    const { data } = useQuery({
        queryKey: ["buys"],
        queryFn: getBuys,
    })
    ...
    const handleSelect = (buys: BuysFormDataInfo) => {
        onSelectionChange(buys)
    }
        ...
        <>
            <Dialog.Root open={open} onOpenChange={setOpen}>
                <Dialog.Trigger asChild>
                    <button
                        className="flex items-center justify-center py-1 px-2 border gap-x-2 border-gray-300  rounded font-medium  text-black outline-none hover:bg-slate-100/65 focus-visible:outline-black transition-all duration-200 w-full md:w-[50%]"
                        onClick={() => {                            
                            setOpen(true)                                                    
                        }}
                    >
                ...
                <Dialog.Portal>
                    ...
                    <Dialog.Content className={`w-[95%] fixed left-1/2 top-1/2 overflow-y-auto scroll-smooth touch-pan-y overflow-scroll -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        <Dialog.Title className="font-bold text-black text-2xl text-center">
                            Selección de los productos a de volver
                        </Dialog.Title>
                        <Dialog.Description className="my-2 text-base text-black text-center">
                        Selecciona el producto que devolveran de esta compra...
                        </Dialog.Description>
                            ...
                            <Table.Root size="1" variant="surface" className="overflow-x-auto h-full overflow-y-auto touch-pan-y touch-pan-x p-2">
                                <Table.Header className="top-0 sticky bg-white">
                                    <Table.Row align="center">
                                        <Table.ColumnHeaderCell>N. compra</Table.ColumnHeaderCell>                                        
                                        <Table.ColumnHeaderCell>N. factura proveedor</Table.ColumnHeaderCell>
                                        <Table.ColumnHeaderCell>Termino</Table.ColumnHeaderCell>
                                        <Table.ColumnHeaderCell>Subtotal</Table.ColumnHeaderCell>
                                        <Table.ColumnHeaderCell>Total</Table.ColumnHeaderCell>
                                        <Table.ColumnHeaderCell>Impuesto</Table.ColumnHeaderCell>
                                ...
                                <Table.Body>
                                    {
                                        filteredBuys?.map(buys => (
                                            <Table.Row align="center" key={buys.id} className="hover:bg-gray-100/85 transition-all duration-200">
                                                {
                                                    <Table.Cell>
                                                        {buys.numero_compra}
                                                    </Table.Cell>
                                                        ...
                                                        <div className="flex gap-2 items-center justify-center">
                                                            <Tooltip content="Clic para seleccionar producto">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => {
                                                                        handleSelect({ ...buys, detalles_compra: [], metodo_pago: [] })                                                                        
                                                                        setOpen(false)
                                                                    }}
                                                                    className="size-5"
                                                                >
