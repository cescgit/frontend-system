...
import { SupplierFormDataDelete } from "../../types/supplierData"
import { deleteSupplier } from "../../apis/SupplierAPI"
import Swal from "sweetalert2";
import withReactContent from 'sweetalert2-react-content'

const MySwal = withReactContent(Swal);

export default function DeleteSupplierAlertDialog({ supplier, onClose }: { supplier: SupplierFormDataDelete, onClose: () => void }) {
    const [deleteFormSupplier] = useState(supplier)
                ...
                <AlertDialog.Portal>
                    ...
                    <AlertDialog.Content className={`fixed left-1/2 top-1/2 max-h-[85vh] w-[90vw] max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        <AlertDialog.Title className="font-bold text-center text-lg">
                            ¿Seguro desea eliminar el proveedor?
                        </AlertDialog.Title>
                        <AlertDialog.Description className="my-2 text-base text-center text-gray-600">
                            {
                                <p>
                                    El proveedor {" "} 
                                    <span className="font-bold">{deleteFormSupplier.nombre_proveedor}</span>, 
                                    se eliminara si haces clic en <br />
                                    <span className="bg-red-200 px-1">Si, eliminar proveedor...</span>
                                </p>
                            }
