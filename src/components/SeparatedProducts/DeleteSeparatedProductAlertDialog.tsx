...
import { cancelSeparatedProduct } from "../../apis/SeparatedProductAPI";
import Swal from "sweetalert2";
import withReactContent from 'sweetalert2-react-content'

const MySwal = withReactContent(Swal);

export default function DeleteSeparatedProductAlertDialog({ separatedProduct, onClose }: { separatedProduct: SeparatedProductFormDataDelete, onClose: () => void }) {
    const [deleteFormSeparatedProduct] = useState(separatedProduct)
    const queryClient = useQueryClient();

    const [open, setOpen] = useState(true);

    const { mutate } = useMutation({
        mutationFn: cancelSeparatedProduct,
        onError: (error) => {
            MySwal.fire({
                position: "center",
                ...
                <AlertDialog.Portal>
                    ...
                    <AlertDialog.Content className={`fixed left-1/2 top-1/2 max-h-[85vh] w-[90vw] max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        <AlertDialog.Title className="font-bold text-center text-lg">
                            ¿Seguro desea anular el apartado?
                        </AlertDialog.Title>
                        <AlertDialog.Description className="my-2 text-base text-center text-gray-600">
                            {
                                <p>
                                    El apartado número {" "}
                                    <span className="font-bold">{deleteFormSeparatedProduct.numero_apartado}</span>,
                                    se anulara si haces clic en <br />
                                    <span className="bg-red-200 px-1">Si, anular apartado...</span>
                                </p>
                            }
                        ...
                        <div className="flex flex-col md:flex-row justify-center md:justify-end gap-y-4 md:gap-x-10">
                            ...
                            <AlertDialog.Action asChild>
                                <button
                                    ...
                                    onClick={() => {
                                        mutate(deleteFormSeparatedProduct.id)
                                        onClose()
                                    }}
                                >
                                    ¡Si, Anularlo!
