...
import { getAllAuxiliaryBook } from "../../apis/AuxiliaryBookAPI"
import ToogleFieldsDialogAuxiliaryBook from "../../components/AuxiliaryBooks/ToogleFieldsDialogAuxiliaryBooks"
import { AuxiliaryBookFormDataInfo } from "../../types/auxiliaryBookData"
import CreateAuxiliaryBook from "../../components/AuxiliaryBooks/CreateAuxiliaryBook"
import EditAuxiliaryBook from "../../components/AuxiliaryBooks/EditAuxiliaryBook"

export default function AuxiliaryBookView({ dataAuth }: { dataAuth: AuthPermissions }) {
    const navigate = useNavigate();
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const modalEditAuxiliaryBook = queryParams.get("editAuxiliaryBook");
    const showEditModal = modalEditAuxiliaryBook ? true : false;

    const { data, isLoading, refetch } = useQuery({
        queryKey: ["auxiliaryBooksdgerRegister"],
        queryFn: getAllAuxiliaryBook,
    })
    ...
    const [editingauxiliaryBook, setEditingauxiliaryBook] = useState<AuxiliaryBookFormDataInfo | null>(null)
    ...
    const filteredAuxiliaryBook = data?.filter(auxiliaryBook =>
                        ...
                        <div className="h-full flex flex-col items-center justify-center w-full px-4">
                            <section className="h-[10%] w-full flex flex-col-reverse md:flex-row items-center justify-center gap-x-10">
                                ...
                                <div className="w-full flex flex-col md:flex-row items-center justify-center gap-y-4 md:gap-x-4">
                                    ...
                                    {
                                        dataAuth?.permisos_contabilidad[0].guardar == 1 && (<CreateAuxiliaryBook />)
                                    }
                            ...
                            <div className="mt-3 w-full h-[23rem] md:h-[28rem] mx-auto">
                                <Table.Root size="1" variant="surface" className="overflow-x-auto h-full overflow-y-auto touch-pan-y touch-pan-x">
                                    ...
                                    <Table.Body>
                                        {
                                            filteredAuxiliaryBook?.map(auxiliaryBook => (
                                                                                ...
                                                                                (
                                                                                    <>
                                                                                        <DropdownMenu.Item
                                                                                            ...
                                                                                            onClick={() => {
                                                                                                setEditingauxiliaryBook(auxiliaryBook)
                                        ...
                                        <Table.Row>
                                            {
                                                filteredAuxiliaryBook?.length == 0 &&
                                                (
...

                                        {
                                            editingauxiliaryBook && (
                                                <Dialog.Root open={openDialogTypeEditAccount} onOpenChange={() => {
                                                    ...
                                                    <EditAuxiliaryBook auxiliaryBook={{ ...editingauxiliaryBook, usuario_modificador: "default_value" }} onClose={() => setEditingauxiliaryBook(null)} />
                                                </Dialog.Root>
                                            )
                                        }
                                    </Table.Body>
