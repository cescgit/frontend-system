...
import { CategoryFormDataDelete } from "../../types/categoryData";
...
export default function DeleteCategoryAlertDialog({ category, onClose }: { category: CategoryFormDataDelete, onClose: () => void }) {
    const [deleteFormCategory] = useState(category)
                ...
                <AlertDialog.Portal>
                    ...
                    <AlertDialog.Content className={`fixed left-1/2 top-1/2 max-h-[85vh] w-[90vw] max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        <AlertDialog.Title className="font-bold text-center text-lg">
                            ¿Seguro desea eliminar el categoría?
                        </AlertDialog.Title>
                        <AlertDialog.Description className="my-2 text-base text-center text-gray-600">
                            {
                                <p>
                                    La categoría {" "} 
                                    <span className="font-bold">{deleteFormCategory.nombre_categoria}</span>, 
                                    se eliminara si haces clic en <br />
                                    <span className="bg-red-200 px-1">Si, eliminar categoría...</span>
                                </p>
                            }
