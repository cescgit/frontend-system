...
export default function CreateSalesFromSalesQuote({ salesQuote, numero_cotizacion, dataAuth }: PropsSalesQuote) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const idSalesQuote = queryParams.get("createBilling")!
    ...
    const [dataProducts, setDataProducts] = useState<TempPurchasingFormData[]>(sessionStorage.getItem("tempProductsDataSalesQuoteSales") ? JSON.parse(sessionStorage.getItem("tempProductsDataSalesQuoteSales")!) : [])
    ...
    useEffect(() => {
        const storedProducts = sessionStorage.getItem("tempProductsDataSalesQuoteSales");
        if (storedProducts) {
    ...
    useEffect(() => {
        sessionStorage.setItem("tempProductsDataSalesQuoteSales", JSON.stringify(dataProducts));
    }, [dataProducts])
    ...
    const { mutate } = useMutation({
        mutationFn: createSalesFromSalesQuote,
        onError: (error) => {
            MySwal.fire({
                position: "center",
                title: "facturación",
        ...
        onSuccess: (data) => {
            ...
            queryClient.invalidateQueries({ queryKey: ["createSales", idSalesQuote] })
            MySwal.fire({
                position: "center",
                title: "Facturación",
                icon: "success",
    ...
    const onSubmitEditSalesQuote = () => {
        ...
        if (customerInput != "") {
            newSales.id_cliente = salesQuote.id_cliente;
        }
        ...
        setNewSales({ ...newSales, subtotal: dataSalesQuote.subtotal });

        const formData = newSales;
        const dataSales = { idSalesQuote, formData }
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content
                    ...
                    <Dialog.Title className="font-bold text-black text-2xl text-center">
                        Convertir proforma en factura
                    </Dialog.Title>
                    <Dialog.Description className="my-2 text-base text-black text-center">
                        Convertir el numero de proforma: <span className="text-cyan-700 font-bold">{numero_cotizacion}</span> en factura...
                    </Dialog.Description>
