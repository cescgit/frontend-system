...
interface AdvanceOrpaymentBalanceProps {
    totalBalance: number;
    id_cliente: string;
    idBalanceCliente: string;
    onClose: () => void
}

export default function AdvanceOrPaymentBalanceCustomer({ totalBalance, id_cliente, idBalanceCliente, onClose }: AdvanceOrpaymentBalanceProps) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const customerBalanceId = queryParams.get("walletCustomer")!;
    let idUSerLogin;    
    ...
    const { mutate } = useMutation({
        mutationFn: createAdvanceOrPaymentCustomer,
        onError: (error) => {
            MySwal.fire({
                position: "center",
                title: "Balance del cliente",
        ...
        onSuccess: (data) => {
            ...
            queryClient.invalidateQueries({ queryKey: ["createAdvanceOrPaymentCustomerrBalance"] })
            MySwal.fire({
                position: "center",
                title: "Balance del cliente",
                icon: "success",
            ...
            onClose();            
            navigate(location.pathname + `?walletCustomer=${idBalanceCliente}`);
        }
    })
    ...
    const {
        ...
        reset,
        formState: { errors },
    } = useForm<CustomerBalanceFormDataCustomerAdd>({ defaultValues: newAdvanceOrPayment });
    ...
    const onSubmitCreateAdvanceOrPayment = () => {
        ...
        newAdvanceOrPayment.id_cliente = id_cliente;
        newAdvanceOrPayment.id_balance_cliente = idBalanceCliente;

        if (isCancel == true) {
            newAdvanceOrPayment.debito = totalBalanceInfo.toString();
        }
        const formData = newAdvanceOrPayment;
        const data = { id_cliente, formData }
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content
                    ...
                    <Dialog.Title className="font-bold text-black text-2xl text-center">
                        Registra anticipo o pago de cliente
                    </Dialog.Title>
                    <Dialog.Description className="my-2 text-base text-black text-center">
                        Aquí puedes registrar un anticipo o pago de un cliente, por favor completa los campos requeridos...
                    </Dialog.Description>
                    ...
                    <Dialog.Close asChild>
                        <button
                            ...
                            aria-label="Close"
                            onClick={() => {
                                navigate(location.pathname + `?walletCustomer=${customerBalanceId}`);
                                reset();
                            }}
                        >
                            <X />
