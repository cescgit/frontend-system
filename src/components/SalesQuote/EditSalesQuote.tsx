...
import { getDetailsSalesQuoteById, updateSalesQuote } from "../../apis/SalesQuoteAPI";
...
interface SalesQuoteProps {
    salesQuote: SalesQuoteFormDataEdit;
    onClose: () => void;
    dataAuth: AuthPermissions;
}

export default function EditSalesQuote({ salesQuote, onClose, dataAuth }: SalesQuoteProps) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const salesQuoteId = queryParams.get("editSalesQuote")!
    ...
    const [dataProducts, setDataProducts] = useState<TempPurchasingFormData[]>(sessionStorage.getItem("tempProductsEditSalesQuote") ? JSON.parse(sessionStorage.getItem("tempProductsEditSalesQuote")!) : [])
    ...
    const [editSalesQuote, setEditSalesQuote] = useState(salesQuote)
    ...
    useEffect(() => {
        ...
        const date = editSalesQuote.fecha_finalizacion.slice(0, 10);

        setEndDate(date)
    }, [dataCustomers, setCustomerName, editSalesQuote.fecha_finalizacion])

    const [customerInput, setcustomerInput] = useState("");
    useEffect(() => {
        const currentLabel = customerName.find(item => item.value === editSalesQuote.id_cliente)?.label || "";
        setcustomerInput(currentLabel);
    }, [editSalesQuote.id_cliente, customerName]);
    ...
    useEffect(() => {
        const storedProducts = sessionStorage.getItem("tempProductsEditSalesQuote");
        if (storedProducts) {
    ...
    useEffect(() => {
        sessionStorage.setItem("tempProductsEditSalesQuote", JSON.stringify(dataProducts));
    }, [dataProducts])
    ...
    function handleCalculateTotal() {
        ...
        editSalesQuote.subtotal = sum.toString();
    ...
    const handleCalculateTotalWithTaxes = () => {
        ...
        editSalesQuote.total = sum.toString();
        ...
        editSalesQuote.total = result.toString();
        setTotal(result)
    }
    ...
    const {
        handleSubmit,
        reset,
    } = useForm<SalesQuoteFormDataEdit>()
    ...
    const onSubmitEditSalesQuote = () => {
        ...
        editSalesQuote.usuario_modificador = idUSerLogin;
        const dataStorage = getDataLocalStorage(dataProducts)

        handleCalculateTotalWithTaxes();
        editSalesQuote.total = total === 0 ? handleCalculateTotal() : total.toString();
        editSalesQuote.detalle_cotizacion_venta = dataStorage;
        editSalesQuote.fecha_finalizacion = endDate;
        editSalesQuote.dias = daysDifference;
        editSalesQuote.impuesto_manual = [{
        ...
        const formData = editSalesQuote;
        const data = { salesQuoteId, formData };
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content
                    ...
                    <Dialog.Description className="my-2 text-base text-black text-center">
                        Modifica tu proforma aquí...
                    </Dialog.Description>

                    <div className="border border-gray-600 rounded-md h-80 sm:mx-auto scrollbar-thin-custom touch-pan-y scroll-smooth w-full px-1 sm:p-4 overflow-scroll">
                        <div className="flex flex-col md:flex-row items-center justify-center gap-x-4 w-full">
                                ...
                                <div className="flex flex-col md:flex-row w-full items-center gap-x-4">
                                    <div className="w-full block">
                                        ...
                                        <input
                                            ...
                                            id="termino"
                                            value={editSalesQuote.termino}
                                            onChange={(e) => setEditSalesQuote({ ...editSalesQuote, termino: e.target.value })}
                                            type="text"
                                    ...
                                    <div className="flex w-full items-start gap-y-1 md:gap-x-2 flex-col">
                                        <div className="w-full block">
                                            ...
                                            <select
                                                onChange={(e) => {
                                                    setEditSalesQuote({ ...editSalesQuote, estado: +e.target.value })
                                                }}
                                                disabled={dataAuth.tipo_usuario == import.meta.env.VITE_TYPEFROM_USER && daysDifference > 0 ? false : true}
                                                defaultValue={editSalesQuote.estado}
                                                name=""
                                                id="estado"
                                                className={`w-full border border-gray-300 hover:border-gray-500 py-1 px-2 outline-none rounded-md ${daysDifference > 0 && dataAuth.tipo_usuario == import.meta.env.VITE_TYPEFROM_USER ? "" : "cursor-not-allowed"}`}>
                                                {
                                                    stateValue.map(item => (
                                ...
                                <div className="flex w-full flex-col md:flex-row items-center gap-x-4 mt-2">
                                    <div className="w-full block">
                                        ...
                                        <textarea
                                            ...
                                            id="observaciones"
                                            value={editSalesQuote.observaciones}
                                            onChange={(e) => setEditSalesQuote({ ...editSalesQuote, observaciones: e.target.value })}
                                            placeholder="Ejemplo: xxxxx..."
                                        />
                                    </div>
                                </div>
                                        ...
                                        <div className="border border-gray-300 hover:border-gray-400 rounded-md py-1 px-2 flex items-center justify-center w-full">
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (selectedItem) {
                                                        setEditSalesQuote({ ...editSalesQuote, id_cliente: selectedItem.value });
                                                    }
                                                }}
                                            />
                            ...
                            <div className="flex flex-col flex-1 w-full items-center mt-4 border border-gray-400 px-1 py-2 sm:p-4 rounded-lg">
                                ...
                                <div className="w-full border border-gray-400 rounded-lg py-1 px-2 flex flex-col md:flex-row gap-y-2 items-center justify-center gap-x-1">
                                    <div className="flex items-center justify-center">
                                        ...
                                        <input
                                            className={`border-gray-400 outline-none focus-visible:border-gray-600 py-1 px-2 rounded-md ${dataAuth.tipo_usuario == import.meta.env.VITE_TYPEFROM_USER ? "" : "cursor-not-allowed"}`}
                                            id="datePickEnd"
                                    ...
                                    <div className="flex items-center justify-center md:space-x-1">
                                        <input
                                            type="text"
                                            value={startDate === "" && endDate === "" ? editSalesQuote.dias : daysDifference}
                                            disabled
                    ...
                    <div className="h-96 w-full flex-col lg:flex-row flex md:gap-x-4 gap-y-4 items-start justify-center top-0">
                        <div className="w-full h-full lg:w-[75%] scrollbar-thin-custom touch-pan-x touch-pan-y scroll-smooth overflow-scroll top-0">
                            <Table.Root size="1" variant="ghost" className="overflow-x-auto h-full overflow-y-auto touch-pan-y touch-pan-x w-7xl md:w-auto">
                                                                                ...
                                                                                html:
                                                                                    <div className="flex flex-col items-center">
                                                                                        <p>
                                                                                            ...
                                                                                            <br />
                                                                                            se quito de la lista de la proforma...
                                                                                        </p>
                                                                                    </div>
                                                                                ,
                                                                                showConfirmButton: false,
                        ...
                        <div className="w-full lg:w-[25%] flex items-end gap-y-2 flex-col mt-2 border border-gray-500 rounded-lg py-4 px-2 h-96 scrollbar-thin-custom touch-pan-y scroll-smooth overflow-scroll">
                            ...
                            <div className="flex flex-col gap-y-2 md:gap-y-0 w-full items-center md:justify-between px-4 sm:px-0 mx-auto">
                                ...
                                <input
                                    id="value_tax"
                                    className="w-full border border-gray-400 hover:border-gray-600 outline-none rounded-md py-1 px-2 cursor-not-allowed"
                                    value={valorImpuesto === 0 ? formatCurrency(salesQuote.impuesto_manual![0].valor_cantidad.toFixed(2)) : formatCurrency(valorImpuesto.toString())}
                                    type="text"
                                    disabled
                    ...
                    <form onSubmit={handleSubmit(onSubmitEditSalesQuote)} className="space-y-4 w-full p-4">
                            ...
                            <button
                                ...
                                disabled={
                                    dataProducts.length <= 0 ||
                                        editSalesQuote.termino == "" ||
                                        editSalesQuote.observaciones == ""
                                        ? true : false}
                            >
