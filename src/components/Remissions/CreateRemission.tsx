...
import { Edit, Plus, PlusIcon, Save, Search, Trash2, X } from "lucide-react";
import { Dialog } from "radix-ui";
import { useForm } from "react-hook-form";
import { useEffect, useState } from "react";
import { JWTData } from "../../types/jwtData";
import { jwtDecode } from "jwt-decode";
import { createRemission } from "../../apis/RemissionsAPI";
import { RemissionsFormDataAdd } from "../../types/remissionsData";
...
export default function CreateRemission() {
    ...
    const [dataProducts, setDataProducts] = useState<TempPurchasingFormData[]>(sessionStorage.getItem("tempProductsRemissionAdd") ? JSON.parse(sessionStorage.getItem("tempProductsRemissionAdd")!) : [])
    ...
    const [newRemission, setNewRemission] = useState({
        id_cliente: "",
        usuario_creador: "",
        detalle_remision: [{
            nombre_producto: "",
    ...
    useEffect(() => {
        const storedProducts = sessionStorage.getItem("tempProductsRemissionAdd");
        if (storedProducts) {
    ...
    useEffect(() => {
        sessionStorage.setItem("tempProductsRemissionAdd", JSON.stringify(dataProducts));
    }, [dataProducts])
    ...
    const {
        handleSubmit,
        reset,
    } = useForm<RemissionsFormDataAdd>({ defaultValues: newRemission });
    ...
    const onSubmitCreateRemission = () => {
        ...
        newRemission.usuario_creador = idUSerLogin;
        const dataStorage = getDataLocalStorage(dataProducts)
        newRemission.detalle_remision = dataStorage;

        if(newRemission.id_cliente == "" || newRemission.id_cliente == undefined || newRemission.id_cliente == null) {
            MySwal.fire({
                position: "center",
                title: "Remisiones",
                icon: "error",
                html:
                    <div className="flex flex-col items-center">
                        <p className="text-red-500 font-bold">
                            Debes de seleccionar el cliente...
                        </p>
                    </div>
                ,
                showConfirmButton: false,
            ...
            <Dialog.Root open={open} onOpenChange={setOpen}>
                <Dialog.Trigger asChild>
                    <button
                        ...
                        onClick={() =>
                            navigate(location.pathname + "?createRemission")
                        }
                    >
                ...
                <Dialog.Portal>
                    ...
                    <Dialog.Content
                        ...
                        className="fixed left-1/2 top-1/2 h-dvh md:h-[95%] scrollbar-thin-custom overflow-y-auto touch-pan-y w-[98%] max-w-[550px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-4 shadow-[var(--shadow-6)] focus:outline-none data-[state=open]:animate-contentShow">
                        ...
                        >
                                ...
                                <div className="border border-gray-300 rounded-md py-1 px-2 flex items-center justify-center w-full">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setNewRemission({ ...newRemission, id_cliente: selectedItem!.value })                                            
                                        }}
                                    />
                            ...
                            <div className="w-full flex items-center justify-center flex-col">
                                ...
                                <div className="flex flex-1 w-full items-center mt-4 border border-gray-400 px-1 sm:p-4 rounded-lg py-4">
                                    <div className="w-full flex items-center justify-center flex-col">
                            ...
                            <div className="h-72 w-full top-0 mx-auto">
                                <div className="w-full h-full scrollbar-thin-custom touch-pan-x touch-pan-y scroll-smooth overflow-scroll top-0">
                                    <Table.Root size="1" variant="ghost" className="overflow-x-auto h-full overflow-y-auto touch-pan-y touch-pan-x">
                            ...
                            <button
                                type="submit"
                                className={`w-full md:w-[50%] mx-auto border border-gray-300 py-2 px-4 bg-slate-50/85 rounded-md flex items-center justify-center gap-x-4 font-bold hover:bg-slate-100/85 transition-all duration-200 ${customerName == undefined || customerName == null || productId == null || productId == undefined || newRemission.id_cliente == "" ? "cursor-not-allowed" : ""}`}
                                aria-label="Close"
                                disabled={customerName == undefined || customerName == null || productId == null || productId == undefined || newRemission.id_cliente == "" ? true : false}
                            >
