...
export default function CustomerComboBoxSales({ onSelectionChange }: { onSelectionChange: (customer: CustomerFormDataInfo) => void }) {    
    ...
    const handleSelect = (customer: CustomerFormDataInfo) => {
                ...
                <Dialog.Portal>
                    ...
                    <Dialog.Content className={`w-[98%] md:w-[80%] fixed left-1/2 top-1/2 overflow-y-auto scroll-smooth touch-pan-y overflow-scroll -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        ...
                        <Dialog.Description className="my-2 text-base text-black text-center">
                            Selecciona el cliente de esta factura...
                        </Dialog.Description>
                                ...
                                <Table.Body>
                                    {
                                        filteredCustomer?.map(customer => (
                                            <Table.Row align="center" key={customer.id} className="hover:bg-gray-100/85 transition-all duration-200">
                                                {
                                                    <Table.Cell>
