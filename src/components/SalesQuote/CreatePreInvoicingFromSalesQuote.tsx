...
interface PropsSalesQuote {
    salesQuote: SalesQuoteFormDataInfo;
    numero_cotizacion: string;
    dataAuth: AuthPermissions
}

export default function CreatePreInvoicingFromSalesQuote({ salesQuote, numero_cotizacion, dataAuth }: PropsSalesQuote) {
    const navigate = useNavigate()
    const location = useLocation()
    const queryParams = new URLSearchParams(location.search)
    const idSalesQuote = queryParams.get("createPreInvoicing")!
    const id = idSalesQuote;
    let idUSerLogin,
        precioVenta: number;
    ...
    const [dataProducts, setDataProducts] = useState<TempPurchasingFormData[]>(sessionStorage.getItem("tempProductsDataSalesQuotePreInvoicing") ? JSON.parse(sessionStorage.getItem("tempProductsDataSalesQuotePreInvoicing")!) : [])
    ...
    const [dataSalesQuote] = useState(salesQuote)
    ...
    const { data } = useQuery({
        queryKey: ["salesQuote", idSalesQuote],
        queryFn: () => getDetailsSalesQuoteById({ id }),
        enabled: !!idSalesQuote,
        retry: false
    })
    ...
    useEffect(() => {
        const currentLabel = customerName.find(item => item.value === dataSalesQuote.id_cliente)?.label || "";
        setCustomerInput(currentLabel);
        setIsCustomerExists(true);
    }, [dataSalesQuote.id_cliente, customerName]);
    ...
    useEffect(() => {
        const storedProducts = sessionStorage.getItem("tempProductsDataSalesQuotePreInvoicing");
        if (storedProducts) {
    ...
    useEffect(() => {
        sessionStorage.setItem("tempProductsDataSalesQuotePreInvoicing", JSON.stringify(dataProducts));
    }, [dataProducts])
    ...
    function handleCalculateTotal() {
        ...
        newPreInvoicing.subtotal = sum.toFixed(2).toString();
        return sum.toString();
    }
    ...
    const handleCalculateTotalWithTaxes = () => {
        ...
        newPreInvoicing.total = sum.toFixed(2).toString();
        ...
        dataSalesQuote.total = result.toFixed(2).toString();
        setTotal(result)
    }
    ...
    const { mutate } = useMutation({
        mutationFn: createPreInvoicingFromSalesQuote,
        onError: (error) => {
            MySwal.fire({
                position: "center",
        ...
        onSuccess: (data) => {
            ...
            queryClient.invalidateQueries({ queryKey: ["createPreInvoicing", idSalesQuote] })
            MySwal.fire({
                position: "center",
                title: "Prefacturación",
                icon: "success",
    ...
    const onSubmitEditSalesQuote = () => {
        ...
        newPreInvoicing.total = total === 0 ? handleCalculateTotal() : total.toString();
        newPreInvoicing.detalles_prefacturacion = dataStorage;
        newPreInvoicing.dias = daysDifference;
        ...
        newPreInvoicing.id_cliente = dataSalesQuote.id_cliente;
        setNewPreInvoicing({ ...newPreInvoicing, subtotal: dataSalesQuote.subtotal });

        if (isCustomerExists == true) {            
            newPreInvoicing.cliente_existente = 1;
        }
        else {            
            newPreInvoicing.cliente_existente = 0;
        }
        ...
        const formData = newPreInvoicing;
        const dataPreInvoicing = { idSalesQuote, formData }        
        mutate(dataPreInvoicing)
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content
                    ...
                    <Dialog.Title className="font-bold text-black text-2xl text-center">
                        Convertir proforma en prefactura
                    </Dialog.Title>
                    <Dialog.Description className="my-2 text-base text-black text-center">
                        Convertir el numero de proforma: <span className="text-cyan-700 font-bold">{numero_cotizacion}</span> en prefactura...
                    </Dialog.Description>
                    ...
                    <div className="border border-gray-600 rounded-md h-80 sm:mx-auto scrollbar-thin-custom touch-pan-y scroll-smooth w-full px-1 sm:p-4 overflow-scroll">
                                ...
                                <div className="flex flex-col md:flex-row w-full items-center gap-x-4">
                                    ...
                                    <div className="flex w-full items-start gap-y-1 md:gap-x-2 flex-col">
                                        <div className="w-full block">
                                            ...
                                            <select
                                                onChange={(e) => {
                                                    setNewPreInvoicing({ ...newPreInvoicing, estado: +e.target.value })
                                                }}
                                                value={newPreInvoicing.estado}
                                                name=""
                                                id="estado"
                                                            ...
                                                            <div className="border border-gray-300 hover:border-gray-400 rounded-md py-1 px-2 flex items-center justify-center w-full">
                                                                <input
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        if (selectedItem) {
                                                                            setNewPreInvoicing({ ...newPreInvoicing, id_cliente: selectedItem.value });
                                                                        }
                                                                    }}
                                                                />
                        ...
                        <div className="flex flex-col md:flex-row items-start w-full justify-center gap-x-4">
                            <div className="flex w-full md:w-[65%] lg:w-[50%] items-center mt-4 border border-gray-400 px-1 py-2 sm:p-4 rounded-lg">
                                <div className="w-full flex items-center justify-center flex-col">
                                    ...
                                    <div className="flex flex-col md:flex-row items-center justify-center w-full gap-y-2 gap-x-0 md:gap-y-0 md:gap-x-2">
                                        <div className="w-full mt-4 gap-Y-2 flex-col flex items-start">
                            ...
                            <button
                                ...
                                disabled={
                                    dataProducts.length <= 0 ||
                                        newPreInvoicing.termino == "" ||
                                        newPreInvoicing.observaciones == ""
                                        ? true : false}
                            >
