...
export default function ReactivateSalesQuoteAlertDialog({ salesQuote, onClose }: { salesQuote: SalesQuoteFormDataDelete, onClose: () => void }) {
    const [reactivateFormSalesQuote] = useState(salesQuote)
    const queryClient = useQueryClient()

    const [open, setOpen] = useState(true);

    const { mutate } = useMutation({
        mutationFn: reactivateSalesQuote,
        onError: (error) => {
            MySwal.fire({
                position: "center",
                title: "Proforma",
                ...
                <AlertDialog.Portal>
                    ...
                    <AlertDialog.Content className={`fixed left-1/2 top-1/2 max-h-[85vh] w-[90vw] max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=${open}]:animate-contentShow`}>
                        <AlertDialog.Title className="font-bold text-center text-lg">
                            ¿Seguro desea reactivar la proforma?
                        </AlertDialog.Title>
                        <AlertDialog.Description className="my-2 text-base text-center text-gray-600">
                            {
                                <p>
                                    La proforma número {" "}
                                    <span className="font-bold">{reactivateFormSalesQuote.numero_cotizacion}</span>,
                                    se reactivara si haces clic en <br />
                                    <span className="bg-cyan-200 px-1">Si, reactivar proforma...</span>
                                </p>
                            }
                            <div className="flex items-center justify-center my-2 md:my-4">
                                <Info className="size-20 text-cyan-500" />
                            </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-center md:justify-end gap-y-4 md:gap-x-10">
                            ...
                            <AlertDialog.Action asChild>
                                <button
                                    type="button"
                                    className="w-full md:w-auto py-1 px-4 rounded-md bg-cyan-500 hover:bg-cyan-600 text-white"
                                    onClick={() => {
                                        mutate(reactivateFormSalesQuote.id)
                                        onClose()
                                    }}
                                >
                                    ¡Si, reactivar!
