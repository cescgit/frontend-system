...
import { KardexData } from "../../../types/kardexData";
...
export default function ReportKardexById({ kardex }: { kardex: KardexData }) {
  const location = useLocation()
  const queryParams = new URLSearchParams(location.search)
  const idProduct = queryParams.get("viewKardex");
  ...
  const downloadPdf = async () => {
    try {
      ...
      const response = await api(`kardex/reportKardex/report/prouct/${idProduct}`, {
      ...
      link.download = `Kardex-${kardex.nombre_producto}.pdf`; // Nombre del archivo a descargar
      link.click();
